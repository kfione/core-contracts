 ...
 */
contract Strategy_CheSwap_Farm  is Ownable, ReentrancyGuard, Pausable {
    ...
    address public strategyFarmAddr = 0x8cddB4CD757048C4380ae6A69Db8cD5597442f7b;
    ...
    address[] public singleEarnToStakePath;




    uint256 public controllerFee = 500; // 70;
    ...
    constructor(
        ...
        token0Address = _addresses[1];
        token1Address = _addresses[2];
        kfiFarmAddress =  _addresses[3];
        govAddress = msg.sender;
        pid = _pid;
        earnedToToken0Path = [earnedAddress,token0Address];
        earnedToToken1Path = [earnedAddress,token1Address];
        singleEarnToStakePath = [earnedAddress,stakeToken];
    ...
    function _farm() internal virtual {
        ...
        IStrategyFarm(strategyFarmAddr).deposit(pid, wantAmt);
    }

    function _unfarm(uint256 _wantAmt) internal virtual {
        IStrategyFarm(strategyFarmAddr).withdraw(pid, _wantAmt);
    }
    ...
    function earn() public virtual whenNotPaused {
        ...
        if (token0Amt > 0 && token1Amt > 0) {
            ...
            ISwapRouter(uniRouterAddress).addLiquidity(
                token0Address,
                token1Address,
                token0Amt,
                token1Amt,
    ...
    modifier blockPer() {
        ...
        if(x>diffBlock){
            uint256 balance = IStrategyFarm(strategyFarmAddr).pendingCherry(pid,address(this));
    ...
    function stakePrice() internal view returns (uint256) {
        return IPrice(priceAddr).lpPrice(stakeToken);
    }
    ...
    function setPid(uint256 _pid) public virtual onlyAllowGov {
        pid = _pid;
    }
    ...
    function setEarnedToToken0Path(address[] memory _address) public virtual onlyAllowGov {
        earnedToToken0Path = _address;
        emit UpdatePath(_address);
    }
    function setEarnedToToken1Path(address[] memory _address) public virtual onlyAllowGov {
    ...
    function setSingleEarnToStakePath(address[] memory _address) public virtual onlyAllowGov {
